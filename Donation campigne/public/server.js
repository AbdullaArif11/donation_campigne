...
// Fetch donation cards data and convert it to JSON
app.get('/get-donation-cards', (req, res) => {
    ...
    renumberIds(err => {
        ...
        db.query(query, (err, results) => {
            if (err) {
                console.error('Error fetching donation cards data:', err);
                return res.status(500).send('Error fetching donation cards data.');
            }
            ...
            fs.writeFile('data.json', jsonData, 'utf8', (err) => {
                if (err) {
                    ...
                    return res.status(500).send('Error saving donation cards to data.json.');
                }
...
// Fetch charity organizations data and convert it to JSON
app.get('/get-charity-organizations', (req, res) => {
    const query = 'SELECT * FROM charity_organizations';
    db.query(query, (err, results) => {
        if (err) {
            console.error('Error fetching charity organizations data:', err);
            return res.status(500).send('Error fetching charity organizations data.');
        }
        ...
        fs.writeFile('data2.json', jsonData, 'utf8', (err) => {
            if (err) {
                console.error('Error writing to data2.json:', err);
                return res.status(500).send('Error saving charity organizations to data2.json.');
            }
            res.send(jsonData);  // Send the JSON response to the client
