...
const Details = () => {
  ...
  const { Title, Description, Text_button_bg, Picture, Category } = cards[id - 1];
  const btnBg = {
  ...
  const AddDonate = (ID) => {
    swal({
      ...
      if (donationAmount) {
        ...
        fetch("http://localhost/Web%20Technology/Donation%20campigne/src/Conponents/Details/save_donation.php", {
          method: "POST",
          headers: {
        ...
        .then(data => {
          if (data.status === "success") {
            ...
            swal("Donation Failed", data.message, "error");
          }
        })
        .catch(error => {
          swal("Donation Failed", "An error occurred while saving the donation: " + error.message, "error");
        });
      } else {
